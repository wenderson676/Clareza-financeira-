          ...
          {isSidebarOpen && (
            <>
              ...
              <motion.div
                ...
                className="fixed top-0 left-0 bottom-0 w-[82%] sm:w-80 bg-white dark:bg-slate-900 z-50 shadow-2xl flex flex-col overflow-hidden border-r border-slate-100 dark:border-slate-800"
              >
                {/* Fixed Premium Header */}
                <div className="p-5 border-b border-slate-100 dark:border-slate-800 pt-10 flex flex-col gap-4 bg-white dark:bg-slate-900 shrink-0">
                  <div className="flex justify-between items-center">
                    <div className="flex items-center gap-2">
                      <div className="w-8 h-8 rounded-xl bg-emerald-600 flex items-center justify-center font-bold text-white shadow-md shadow-emerald-500/25">
                        <Sparkles size={16} />
                      </div>
                      <h2 className="text-lg font-extrabold text-slate-800 dark:text-slate-100 tracking-tight">Clareza</h2>
                    </div>
                    <button 
                      onClick={() => setIsSidebarOpen(false)} 
                  ...
                  {state.userName && (
                    <div className="flex items-center gap-3 bg-slate-50 dark:bg-slate-950 p-3 rounded-2xl border border-slate-100 dark:border-slate-800/60 shadow-sm">
                      <div className="w-10 h-10 bg-gradient-to-tr from-emerald-600 to-teal-500 text-white rounded-xl flex items-center justify-center font-extrabold text-base shadow-inner shrink-0">
                        {state.userName.charAt(0).toUpperCase()}
                      </div>
                      <div className="flex-1 min-w-0">
                        <p className="text-[10px] text-slate-400 dark:text-slate-500 font-bold uppercase tracking-wider">Acessando como</p>
                        <p className="font-bold text-xs text-slate-800 dark:text-slate-200 truncate">{state.userName}</p>
                      </div>
                    </div>
                  )}
                </div>
                
                {/* Scrollable Navigation Options grouped by category */}
                <div className="flex-1 overflow-y-auto px-4 py-4 space-y-5 scrollbar-thin">
                  
                  {/* PREFERENCES SECTION */}
                  <div className="space-y-2">
                    <h3 className="px-3 text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Preferências</h3>
                    <div className="space-y-1">
                      <button 
                        onClick={() => setIsDarkMode(!isDarkMode)}
                        className="w-full flex items-center justify-between px-3 py-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors text-left cursor-pointer"
                      >
                        ...
                        <div className="w-8 h-4.5 bg-slate-200 dark:bg-slate-700 rounded-full relative transition-colors">
                          <div className={`absolute top-0.5 left-0.5 w-3.5 h-3.5 rounded-full bg-white transition-transform duration-205 ${isDarkMode ? 'translate-x-3.5 bg-emerald-500' : ''}`} />
                        </div>
                      </button>
                      ...
                      <div className="px-3 py-2 flex flex-col gap-1.5">
                        <div className="flex items-center gap-2.5 text-slate-700 dark:text-slate-200 font-semibold text-xs">
                          ...
                          <span>Divisão de Orçamento</span>
                        </div>
                        <select
                          value={state.budgetMode || '50-30-20'}
                          onChange={(e) => setBudgetMode(e.target.value as BudgetMode)}
                          className="w-full mt-1 px-2.5 py-2 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-xl text-slate-700 dark:text-slate-300 text-[11px] font-bold focus:outline-none focus:ring-1 focus:ring-emerald-500/50"
                        >
                  ...
                  {/* BACKUP & DATA SECTION */}
                  <div className="space-y-2">
                    <h3 className="px-3 text-[10px] font-extrabold text-slate-400 dark:text-slate-500 uppercase tracking-widest">Segurança e Dados</h3>
                    <div className="space-y-1">
                      <button 
                        onClick={handleExportData}
                        className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/60 text-slate-700 dark:text-slate-200 font-semibold text-xs transition-colors text-left cursor-pointer"
                      >
                        <Download size={16} className="text-slate-400" />
                        <span>Fazer Backup (Exportar)</span>
                      </button>

                      <label className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/60 text-slate-700 dark:text-slate-200 font-semibold text-xs transition-colors text-left cursor-pointer">
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="space-y-1">
                      ...
                      >
                        ...
                        <span>Como usar o App (Tutorial)</span>
                      </button>
                      ...
                      <button 
                        ...
                        className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800/60 text-slate-700 dark:text-slate-200 font-semibold text-xs transition-colors text-left cursor-pointer"
                      >
                        <Smartphone size={16} className="text-indigo-400 dark:text-indigo-500" />
                        <span>Instalar como Aplicativo (APK)</span>
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-2 pt-2 border-t border-slate-100 dark:border-slate-800/60">
                    <button 
                      ...
                      className="w-full flex items-center gap-2.5 px-3 py-2.5 rounded-xl hover:bg-rose-50 dark:hover:bg-rose-950/20 text-rose-600 dark:text-rose-400 font-bold text-xs transition-colors text-left cursor-pointer"
                    >
                      <Trash2 size={16} />
                ...
                {/* Permanent Creator Copyright Footer */}
                <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-950/20 flex flex-col items-center justify-center gap-1.5 shrink-0">
                  <div className="flex items-center gap-1 bg-emerald-50 dark:bg-emerald-500/10 text-emerald-600 dark:text-emerald-400 px-2 py-0.5 rounded-full text-[9px] font-extrabold tracking-wider uppercase border border-emerald-100/50 dark:border-emerald-500/20">
                    <Sparkles size={10} className="text-emerald-500 animate-pulse" />
                    <span>Criador do Projeto</span>
                  </div>
