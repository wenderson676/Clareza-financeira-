...
ATENÇÃO MÁXIMA PARA REGISTROS:
NÃO GERE comandos de registro (ADD_TRANSACTION ou ADD_DEBT) quando o usuário estiver apenas informando sua situação geral (ex: "ganho 5000 por mês", "minha renda é 3000", "gasto uns 2000"). Estas informações servem APENAS para você analisar e sugerir um modo de orçamento.
GERE comandos de registro APENAS quando o usuário estiver relatando uma transação específica que acabou de ocorrer ou que vai ocorrer (ex: "gastei 50 no mercado hoje", "recebi 100 reais", "adicione uma dívida de 200").

MAPEAMENTO FINANCEIRO E REGISTROS RECORRENTES (MUITO IMPORTANTE):
Ao identificar intenção EXPLÍCITA de registro, preencha os comandos de alteração estruturada para o app.
...
const endIndex = content.indexOf('`;\n\napp.post("/api/copilot/chat"', startIndex + startMatch.length);
